...
{
    if(totalCidades >= MAX_CIDADES) return 0;
    if(cidade.idCidade <= 0) return 0;
    if(strlen(cidade.nome)==0) return 0;
    if(existeCidade(cidade.idCidade)) return 0;
...
{
    if(totalRotas >= MAX_ROTAS) return 0;

    if(!existeCidade(rota.origem)) return 0;
    if(!existeCidade(rota.destino)) return 0;

    if(rota.origem == rota.destino) return 0;
    if(rota.distancia <= 0) return 0;

    if(existeRota(rota.origem, rota.destino)) return 0;
...
{
    if(!existeRota(origem,destino)) return 0;
